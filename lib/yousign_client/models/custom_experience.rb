  ...
  class CustomExperience
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `YousignClient::CustomExperience` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `YousignClient::CustomExperience`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @button_color.nil?
        invalid_properties.push('invalid value for "button_color", button_color cannot be nil.')
      end

      if @text_color.nil?
        invalid_properties.push('invalid value for "text_color", text_color cannot be nil.')
      end

      if @text_button_color.nil?
        invalid_properties.push('invalid value for "text_button_color", text_button_color cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @button_color.nil?
      return false if @text_color.nil?
      return false if @text_button_color.nil?
    ...
    def hash
      [id, name, landing_page_disabled, side_panel_disabled, background_color, button_color, text_color, text_button_color, disabled_notifications, email_logo_disabled, email_header_text_disabled, email_footer_signature_disabled, email_expiration_text_disabled, redirect_urls, logo, source, created_at].hash
    end
