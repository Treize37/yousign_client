...
module YousignClient
  class SignatureRequestsSignatureRequestIdBody
    attr_accessor :name
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `YousignClient::SignatureRequestsSignatureRequestIdBody` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `YousignClient::SignatureRequestsSignatureRequestIdBody`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      delivery_mode_validator = EnumAttributeValidator.new('Object', ['none', 'email'])
    ...
    def delivery_mode=(delivery_mode)
      validator = EnumAttributeValidator.new('Object', ['none', 'email'])
    ...
    def hash
      [name, delivery_mode, ordered_signers, reminder_settings, timezone, email_custom_note, expiration_date, external_id, branding_id, custom_experience_id, workspace_id].hash
    end
